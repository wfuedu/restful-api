...
Service-specific adapters may be added to a 'restfulApiServiceAdapters' HashMap registered within the Spring container.  For example, the following configuration would adapt the 'fooService' with the 'fooServiceAdapter'. All other services would continue to use a built-in, pass-through adapter.

```groovy
beans = {
    xmlns util: "http://www.springframework.org/schema/util"

    fooServiceAdapter(FooServiceAdapter)

    util.map(id: 'restfulApiServiceAdapters', 'map-class':"java.util.HashMap") {
        entry(key: 'fooService') { ref(bean:'fooServiceAdapter') }
    }
}
```

An entry within the above 'restfulApiServiceAdapters' with a key of 'any' will adapt any services that are not configured with a specific adapter.

Alternatively, a single shared adapter may be configured separately using a bean name of 'restfulServiceAdapter' as in the following example. This single adapter does not preclude the configuration of service-specific adapters as above, and is simply an alternative to using an 'any' entry in the 'restfulApiServiceAdapters' HashMap. _(Support for a separate 'restfulServiceAdapter' is retained for backward compatibility.)_
...
If no adapter is registered, a built-in pass-through adapter is used. Configuring a service adapter within Spring is only required when delegating to services whose contract differs from that expected by this plugin.
